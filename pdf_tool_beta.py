...
class ImageToPDFConverter:
    ...
    def __init__(self, root):
        ...
        # Image management with rotation support
        self.image_rotations = {}  # Track rotation angles by index {index: angle}
        self.original_images = {}  # Cache original images by index
        ...
        self._current_conversion_type = 'original'
    ...
    def create_control_panel(self, parent):
        ...
        self.output_preview_label.grid(row=2, column=0, columnspan=3, sticky="w", padx=5, pady=(0, 5))
        
        # Bind to update preview when filename changes
        self.output_filename.trace_add('write', self.update_output_preview)
        ...
        self.quality_var.trace_add('write', update_quality_description)
    ...
    def create_preview_area(self, parent):
        ...
        preview_frame = ttk.LabelFrame(parent, text="Image Preview (Drag page numbers to reorder, Use rotation buttons)", padding="10")
    ...
    def process_image_safely(self, image_path, index):
        """Process image handling all edge cases including EXIF, alpha channels, etc."""
        try:
            # Open image
            img = Image.open(image_path)
            
            # Handle EXIF orientation
            img = self.handle_exif_orientation(img)
            
            # Store original for rotation purposes (by index)
            if index not in self.original_images:
                self.original_images[index] = img.copy()
            
            # Get current rotation if any (by index)
            current_rotation = self.image_rotations.get(index, 0)
            if current_rotation != 0:
                # Use the original image for rotation to avoid quality loss
                img = self.original_images[index].copy()
                img = img.rotate(-current_rotation, expand=True, fillcolor='white')
            
            return img
            
        except Exception as e:
            raise Exception(f"Failed to process image {os.path.basename(image_path)}: {str(e)}")

    def handle_exif_orientation(self, img):
    ...
    def convert_to_rgb_safe(self, img):
        """Convert image to RGB handling all color modes and alpha channels"""
        ...
        if img.mode == 'RGB':
            ...
            if 'transparency' in img.info:
                ...
                return self.convert_to_rgb_safe(img)
            else:
            ...
            # Handle black and white or grayscale
            return img.convert('RGB')
    ...
    def rotate_image(self, index, angle):
        """Rotate image at specific index by given angle"""
        # Update rotation angle for this specific index
        current_rotation = self.image_rotations.get(index, 0)
        new_rotation = (current_rotation + angle) % 360
        if new_rotation == 0:
            # Remove from dict if back to original
            self.image_rotations.pop(index, None)
        else:
            self.image_rotations[index] = new_rotation
        
        # Only update this specific image widget, not all of them
        self.update_single_image(index)
        
        self.update_status(f"Rotated image #{index + 1} by {angle}°")

    def update_single_image(self, index):
        """Update only a single image widget without refreshing entire display"""
        if index >= len(self.image_paths) or index >= len(self.image_widgets):
            return
            
        # Refresh the entire display for simplicity and reliability
        self.display_selected_images()

    def clear_image_cache(self):
        """Clear cached images to prevent memory leaks"""
        for img in self.original_images.values():
    ...
    def select_images_0ld(self):
        ...
        if filenames:
            ...
            # Clear rotation data when new images are selected
    ...
    def select_images(self):
        """Image selection."""
        filetypes = (
            ("Image files", " ".join([f"*{fmt}" for fmt in self.SUPPORTED_FORMATS])),
            ("All files", "*.*")
        )
        
        try:
            filenames = filedialog.askopenfilenames(
                title="Select Images for PDF Conversion",
                initialdir=self.output_directory.get(),
                filetypes=filetypes
            )
            ...
            messagebox.showerror("Selection Error", f"Failed to select images:\n{str(e)}")
            
    def display_selected_images(self):
        """Display thumbnails of selected images with delete and rotate buttons"""
        # Clear previous content
        for widget in self.images_frame.winfo_children():
            widget.destroy()
        
        # Clear references but keep rotation data if images haven't changed
        self.thumbnail_refs.clear()
        ...
        for idx, image_path in enumerate(self.image_paths):
            try:
                ...
                # Store the image path and index in the widget
                main_container.image_path = image_path
                main_container.image_index = idx
                ...
                # Load and process image with index
                img = self.process_image_safely(image_path, idx)
                
                # Create thumbnail
                display_img = img.copy()
                display_img.thumbnail((200, 200), Image.Resampling.LANCZOS)
                
                # Convert to RGB for display
                display_img = self.convert_to_rgb_safe(display_img)
                ...
                img_label.image = photo  # Keep additional reference
                ...
                rotation_frame = ttk.Frame(main_container)
                rotation_frame.pack(fill="x", pady=2)
                
                # Rotate left button - pass index, not path
                rotate_left_btn = tk.Button(
                    rotation_frame,
                    text="↺ 90°",
                    command=lambda i=idx: self.rotate_image(i, -90),
                    bg="#e6f2ff",
                    font=("Arial", 9),
                    cursor="hand2",
                    width=6
                ...
                # Rotate right button - pass index, not path
                ...
                rotate_right_btn.pack(side="right", padx=2)
                
                # Current rotation indicator
                rotation_angle = self.image_rotations.get(idx, 0)
                if rotation_angle != 0:
                    rotation_label = ttk.Label(
                        rotation_frame,
                        text=f"Rotated: {rotation_angle}°",
                        font=("Arial", 8),
                        foreground="blue"
                    )
                    rotation_label.pack()
                ...
                rotate_left_btn.bind("<Enter>", lambda e, btn=rotate_left_btn: btn.config(bg="#cce6ff"))
                rotate_left_btn.bind("<Leave>", lambda e, btn=rotate_left_btn: btn.config(bg="#e6f2ff"))
                
                rotate_right_btn.bind("<Enter>", lambda e, btn=rotate_right_btn: btn.config(bg="#cce6ff"))
                rotate_right_btn.bind("<Leave>", lambda e, btn=rotate_right_btn: btn.config(bg="#e6f2ff"))
                ...
                # Update grid position with dynamic columns
                col += 1
        ...
        # Schedule a layout refresh after images are displayed
        self.root.after(100, self.refresh_image_layout)
    ...
    def delete_image_with_fast_update(self, index):
        ...
        if 0 <= index < len(self.image_paths):
            ...
            # Clean up cached data for this index
            if index in self.original_images:
                if hasattr(self.original_images[index], 'close'):
                    self.original_images[index].close()
                del self.original_images[index]
            
            # Update rotation data and cached images for remaining images
            new_rotations = {}
            new_originals = {}
            
            for old_idx, angle in self.image_rotations.items():
                if old_idx < index:
                    new_rotations[old_idx] = angle
                elif old_idx > index:
                    new_rotations[old_idx - 1] = angle
                    
            for old_idx, img in self.original_images.items():
                if old_idx < index:
                    new_originals[old_idx] = img
                elif old_idx > index:
                    new_originals[old_idx - 1] = img
                    
            self.image_rotations = new_rotations
            self.original_images = new_originals
    ...
    def calculate_optimal_columns(self):
        ...
        try:
            ...
            available_width = canvas_width - 40  # 40px for padding and scrollbar
    ...
    def reorder_images_fast(self, from_index, to_index):
        ...
        if 0 <= from_index < len(self.image_paths) and 0 <= to_index <= len(self.image_paths):
            ...
            # Update rotation and cache indices
            old_rotations = self.image_rotations.copy()
            old_originals = self.original_images.copy()
            self.image_rotations.clear()
            self.original_images.clear()
            ...
            for i in range(len(self.image_paths)):
                if i == to_index:
                    ...
                    if from_index in old_rotations:
                        self.image_rotations[i] = old_rotations[from_index]
                    if from_index in old_originals:
                        self.original_images[i] = old_originals[from_index]
                else:
                    # Map old index to new index
                    old_idx = i
                    if i > to_index and from_index < to_index:
                        old_idx = i - 1
                    elif i < to_index and from_index > to_index:
                        old_idx = i + 1
                    elif i >= to_index and from_index < i:
                        old_idx = i + 1
                        
                    if old_idx in old_rotations:
                        self.image_rotations[i] = old_rotations[old_idx]
                    if old_idx in old_originals:
                        self.original_images[i] = old_originals[old_idx]
    ...
    # File Existence Checking Methods
    def check_file_exists_and_confirm(self, output_path, is_multiple=False):
        """Check if file exists and get user confirmation"""
        if is_multiple:
            # For multiple files, check if any exist
            existing_files = []
            for idx, image_path in enumerate(self.image_paths):
                base_name = os.path.splitext(os.path.basename(image_path))[0]
                if self.pdf_option.get() == "multiple":
                    if hasattr(self, '_current_conversion_type') and self._current_conversion_type == 'a4':
                        filename = f"{self.output_filename.get()}_{idx+1}_{base_name}.pdf"
                    else:
                        filename = f"{self.output_filename.get()}_{idx+1}_{base_name}.pdf"
                
                check_path = os.path.join(self.output_directory.get(), filename)
            ...
            if existing_files:
                ...
                response = messagebox.askyesnocancel(
                    "Files Already Exist",
                    f"The following PDF files already exist:\n\n{file_list}\n\n"
                    "Do you want to overwrite them?\n\n"
                    "• Yes - Overwrite existing files\n"
                    "• No - Cancel and rename\n"
    ...
    def suggest_alternative_filename(self, original_path):
        """Suggest an alternative filename if the original exists"""
        directory = os.path.dirname(original_path)
        filename = os.path.basename(original_path)
        name, ext = os.path.splitext(filename)
        
        # Try adding numbers until we find a non-existing filename
        counter = 1
        while True:
    ...
    def highlight_filename_entry(self):
        """Briefly highlight the filename entry to show it was updated"""
        original_bg = self.filename_entry.cget("background")
        self.filename_entry.configure(background="#ffffcc")  # Light yellow
        self.root.after(2000, lambda: self.filename_entry.configure(background=original_bg))
    ...
    def update_output_preview(self, *args):
        ...
        if self.pdf_option.get() == "single":
            preview = f"Output: {filename}.pdf"
        else:
            preview = f"Output: {filename}_1_imagename.pdf, {filename}_2_imagename.pdf, ..."
        
        self.output_preview_label.config(text=preview)

    def pre_conversion_check(self):
        """Perform pre-conversion checks including file existence"""
        # Determine the output path(s) based on settings
        if self.pdf_option.get() == "single":
            if self._current_conversion_type == 'a4':
            ...
            # Check single file
            response = self.check_file_exists_and_confirm(output_path, is_multiple=False)
            
            if response == False:  # User wants to rename
                # Suggest alternative name
                suggested = self.suggest_alternative_filename(output_path)
                name_without_ext = os.path.splitext(suggested)[0]
                # if self._current_conversion_type == 'a4' and name_without_ext.endswith('_A4'):
                #     name_without_ext = name_without_ext[:-3]  # Remove _A4 suffix
                self.output_filename.set(name_without_ext)
                self.highlight_filename_entry()
                
                # Show rename dialog
                messagebox.showinfo(
                    "Filename Updated",
                    f"Filename has been updated to: {name_without_ext}\n\n"
                    "Click the Convert button again to proceed with the new filename."
                )
                return False
            elif response == None:  # User cancelled
                return False
        else:
            # Check multiple files
            response = self.check_file_exists_and_confirm("", is_multiple=True)
            
            if response == False:  # User wants to rename
                # Suggest alternative base name
                if self._current_conversion_type == 'a4':
                    test_filename = f"{self.output_filename.get()}_1_test.pdf"
                else:
                    test_filename = f"{self.output_filename.get()}_1_test.pdf"
                test_path = os.path.join(self.output_directory.get(), test_filename)
                
                suggested = self.suggest_alternative_filename(test_path)
                # Extract the base name from suggested filename
                base_name = suggested.split('_1_')[0]
                self.output_filename.set(base_name)
                self.highlight_filename_entry()
                
                messagebox.showinfo(
                    "Filename Updated",
                    f"Base filename has been updated to: {base_name}\n\n"
                    "Click the Convert button again to proceed with the new filename."
                )
                return False
            elif response == None:  # User cancelled
    ...
    def convert_to_pdf(self):
        ...
        # Set conversion type flag
        self._current_conversion_type = 'original'
        
        # Check if we should proceed
        if not self.pre_conversion_check():
            return
    ...
    def convert_to_a4_pdf(self):
        ...
        # Set conversion type flag
        self._current_conversion_type = 'a4'
    ...
    def show_wait_window(self):
        ...
        self.wait_win.title("Please wait...")
        self.wait_win.geometry("200x120")
        self.wait_win.resizable(False, False)
        self.wait_win.transient(self.root)
        self.wait_win.grab_set()  # Make it modal
        self.wait_win.protocol("WM_DELETE_WINDOW", lambda: None)  # Prevent closing
        ttk.Label(self.wait_win, text="Converting...").pack(pady=10)

        self.canvas = tk.Canvas(self.wait_win, width=50, height=50)
        self.canvas.pack()
        self.angle = 0
        self.animate_circle()
    ...
    def animate_circle(self):
        ...
        # Draw a partial arc for animation
        self.canvas.create_arc(x0, y0, x1, y1, start=self.angle, extent=90, style=tk.ARC, width=5)
        self.angle = (self.angle + 10) % 360
        self.anim_job = self.wait_win.after(40, self.animate_circle)
    ...
    def _perform_conversion(self, use_a4=False):
        ...
        try:
            ...
            self.root.after(0, lambda: messagebox.showinfo("Converting...", "Converting files"))
            ...
            self.root.after(0, lambda: messagebox.showerror(f"Conversion Error", {e}))
        finally:
            #self.root.after(0,self.close_wait_window())
    ...
    def _create_single_pdf(self):
        """Create a single PDF from all images with original dimensions and rotations"""
        try:
            # Prepare images
            images = []
            for idx, path in enumerate(self.image_paths):
                # Process image with rotation
                img = self.process_image_safely(path, idx)
                
                # Convert to RGB if necessary
                img = self.convert_to_rgb_safe(img)
                images.append(img)
    ...
    def _create_single_a4_pdf(self):
        """Create a single PDF with all images fitted to A4 pages with rotations"""
        try:
            # Prepare images
            a4_images = []
    ...
    def _create_multiple_pdfs(self):
        """Create individual PDFs for each image with original dimensions and rotations"""
        try:
            created_files = []
    ...
    def _create_multiple_a4_pdfs(self):
        """Create individual A4 PDFs for each image with rotations"""
        try:
            created_files = []
